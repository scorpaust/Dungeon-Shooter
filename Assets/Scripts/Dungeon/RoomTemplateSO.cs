...
public class RoomTemplateSO : ScriptableObject
{
    ...
    #region Tooltip
    [Tooltip("The battle music SO when the room has not been cleared of enemies")]
    #endregion Tooltip
    public MusicTrackSO battleMusic;

    #region Tooltip
    [Tooltip("The ambient music SO when the room has been cleared of enemies")]
    #endregion Tooltip
    ...
    private void OnValidate()
    {
        ...
        if (enemiesByLevelList.Count > 0 || roomEnemySpawnParametersList.Count > 0)
		{
            ...
            foreach (RoomEnemySpawnParameters roomEnemySpawnParameters in roomEnemySpawnParametersList)
			{
				...
				{
                    Debug.Log("No enemy types specified in for the dungeon level " + roomEnemySpawnParameters.dungeonLevel.levelName + " in game object " + this.name.ToString());
				}
			}
		}
